...
(
    SEQUENCE_ID_TYPE SMALLINT NOT NULL,
    LOGICAL_SHARD_ID INT NOT NULL,
    NEXT_VALUE BIGINT NULL,
    SERVER_ID_FLAG TINYINT NOT NULL,
    PRIMARY KEY (SEQUENCE_ID_TYPE, LOGICAL_SHARD_ID, SERVER_ID_FLAG)
) ENGINE=INNODB;

CREATE PROCEDURE GET_SEQUENCE_ID_NEXT_VALUE(
  IN VAR_SEQUENCE_ID_TYPE SMALLINT,
...
BEGIN
  ...
  WHERE SEQUENCE_ID_TYPE = VAR_SEQUENCE_ID_TYPE AND LOGICAL_SHARD_ID = VAR_LOGICAL_SHARD_ID
  FOR UPDATE;
  ...
  INSERT INTO SEQUENCE_ID_NEXT_VALUE (SEQUENCE_ID_TYPE, LOGICAL_SHARD_ID, NEXT_VALUE, SERVER_ID_FLAG)
  VALUES (VAR_SEQUENCE_ID_TYPE, VAR_LOGICAL_SHARD_ID, VAR_NEXT_VALUE + VAR_BATCH_SIZE * 2, VAR_SERVER_ID_FLAG)
